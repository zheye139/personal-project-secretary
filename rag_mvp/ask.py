...
API_ASK_MODES = {"keyword", "vector", "hybrid"}
API_ASK_MIN_LIMIT = 1
API_ASK_MAX_LIMIT = 10
API_ASK_MAX_QUESTION_CHARS = 1000
...
def search_context(
    ...
    不需要修改。
    """
    client = vector_store_config.get_qdrant_client(timeout=120)
...
def normalize_ask_limit(limit: int) -> int:
    """
    Clamp API ask limits to a small local-read range.
    """
    try:
        value = int(limit)
    except Exception:
    ...
    return max(API_ASK_MIN_LIMIT, min(API_ASK_MAX_LIMIT, value))


def safe_api_text(value) -> str:
    """
    Return a string safe for Ask API metadata fields.
    """
...
def format_context_source_for_api(context: dict) -> dict:
    """
    Convert an internal Ask context into public API-safe source metadata.
    """
    return {
        "title": safe_api_text(context.get("title", "")),
        "file_name": safe_api_text(context.get("file_name", "")),
        "project": safe_api_text(context.get("project", "")),
        "doc_type": safe_api_text(context.get("doc_type", "")),
        "category": safe_api_text(context.get("category", "")),
        "tags": safe_api_tags(context.get("tags", [])),
        "score": float(context.get("score", 0.0)),
        "mode": safe_api_text(context.get("mode", "")),
        "chunk_index": context.get("chunk_index", ""),
        "updated_at": safe_api_text(context.get("updated_at", "")),
    }
...
def ask_question(
    ...
    """
    Run retrieval and answer generation, returning an API-safe response.
    """
    normalized_question = str(question or "").strip()
    normalized_mode = str(search_mode or "").strip().lower()
    normalized_limit = normalize_ask_limit(limit)

    if not normalized_question:
        raise ValueError("Ask question is required.")

    if len(normalized_question) > API_ASK_MAX_QUESTION_CHARS:
        raise ValueError("Ask question is too long.")

    if normalized_mode not in API_ASK_MODES:
        raise ValueError("Unsupported ask search mode.")

    if save_log:
        raise ValueError("QA log saving is not enabled for the API.")
    ...
    try:
        ...
        answer = generate_answer(normalized_question, contexts)
    except ValueError:
        raise
    except Exception as exc:
        raise RuntimeError("Ask service is unavailable.") from exc

    sources = [
        format_context_source_for_api(context)
